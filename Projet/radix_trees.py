...
class RadixTree(object):
    ...
    def insert_node(self, key, val, start_node=None):
        ...
        if not current:
            ...
            cont = Container(data=set(val), tag=0)
        ...
        while tested < current._key_size:
            if tested > len(key) - 1:
                ...
                #                  ┗━━━━━━━━━━━━┛
                cont = Container(data=set(val), tag=0)
                cont2 = Container(data=set(val), tag=0)
                node1 = Node(current._key, current._key_size, None)
                node1._next = current._next.copy()
                node1._data = current._data
...
def test():
    ...
    t.insert_node("AT",'B')
    #t.dump()
    t.insert_node("ATCGCG",'C')
    #t.dump()
    t.insert_node("BAB",'A')
    #t.dump()
    t.insert_node("BABCG",'A')
    #t.dump()
    t.insert_node("BABT",'T')
    t.dump()
    return t
